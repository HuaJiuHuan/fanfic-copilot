...
export default memo(function ReadingView() {
  ...
  const stats = useMemo(() => {
    ...
    const writtenScenes = outline.acts.reduce(
        ...
        <div className="flex items-center gap-4">
          {stats && (
            <div className="hidden sm:flex items-center gap-4 text-xs text-academia-muted">
              <span>
                已写 <span className="text-academia-gold font-bold">{stats.writtenScenes}</span>/
                {stats.totalScenes} 场景
              </span>
              <span className="text-academia-border">|</span>
              <span>
                总计{' '}
                <span className="text-academia-gold font-bold">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar pr-4 pb-10">
        <article className="max-w-2xl mx-auto space-y-12">
          <header className="text-center space-y-4 pb-8 border-b border-academia-border/50">
            <h1 className="text-3xl font-serif font-bold text-academia-gold">{outline.title}</h1>
            {outline.logline && (
