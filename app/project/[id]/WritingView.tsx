...
import type { GeneratingPhase } from '@/lib/generate-phase-machine';
  ...
  return (
    <div className="w-full h-full flex flex-col space-y-4 animate-fade-in-up">
      ...
      <div className="flex-1 flex gap-6 h-[600px]">
        ...
        <SceneEditorPanel activeSceneId={activeSceneId} activeSceneInfo={activeSceneInfo} />
      </div>
    </div>
  );
});
...
const SceneEditorPanel = memo(function SceneEditorPanel({
  ...
  const draftContent = useWorkspaceStore((s) =>
    activeSceneId ? s.draftsMap[activeSceneId] || '' : '',
  );
      ...
      <div className="bg-academia-surface/50 border border-academia-border rounded-lg p-3 space-y-3">
        ...
        <div className="flex gap-3">
          <div className="flex-1">
            <label
              htmlFor="scene-word-count"
              className="text-[10px] text-academia-muted block mb-1"
            >
              指定字数
            </label>
            <input
        ...
        {phases.map((p, i) => {
          ...
          return (
            <div key={p} className="flex items-center gap-2 text-xs">
              ...
              {isActive && <span className="w-2 h-4 bg-academia-gold animate-blink ml-auto" />}
            </div>
          );
        })}
      </div>
        ...
        <div className="px-4 pb-3 space-y-1.5">
          {review.issues.map((issue, i) => (
            <div key={i} className="flex items-start gap-2 text-[11px]">
              <span
                className={`shrink-0 px-1.5 py-0.5 rounded border text-[10px] ${SEVERITY_COLORS[issue.severity] || ''}`}
              >
