...
const createProjectStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const { userId, title, fandom, characters, premise, storyText } = inputData;
    ...
    return { projectId, userId, title, fandom, characters, premise, storyText };
  },
});
...
请根据以上全文，反向推导出这篇小说的结构化大纲。`,
    ...
    return { ...inputData, outline: result.object };
  },
});
...
const saveOutlineStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const { projectId, outline } = inputData;
    ...
    return { ...inputData, outlineId: inserted.id };
  },
});
...
const splitTextToScenesStep = createStep({
  ...
  execute: async ({ inputData }) => {
    const { outline, storyText } = inputData;
    const allScenes = outline.acts.flatMap(
      (act: {
        actTitle: string;
        scenes: { id: string; sceneNumber: number; plotAction: string }[];
      }) =>
...
请将原文按上述场景拆分，每个场景截取对应的原文段落。`,
    ...
    return { ...inputData, segments: result.object.segments };
  },
});
...
const saveDraftsStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    if (segments && segments.length > 0) {
      const draftValues = segments.map((seg: { sceneId: string; content: string }) => ({
        projectId,
        outlineId,
        sceneId: seg.sceneId,
        content: seg.content,
